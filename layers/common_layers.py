...
class PositionalEncodingSC(nn.Module):
    def __init__(self, d_model, dropout=0.5, max_len=5000, batch_first=False):
...
class GateModule(nn.Module):
    def __init__(self, input_dim, out_dim, mode='mix'):
        super(GateModule, self).__init__()
        self.input_trans = MLP(input_dim, input_dim)
        self.target_trans = MLP(input_dim, input_dim)
        self.mode = mode
        if self.mode == 'mix':
            self.fc_out = MLP(input_dim * 2, out_dim)
        elif self.mode == 'trans':
            self.fc_out = MLP(input_dim, out_dim)
        self.sigmoid = nn.Sigmoid()

    def get_out_emb(self, input_emb, target_emb):
        if self.mode == 'mix':
            out = self.fc_out(torch.cat((input_emb, target_emb), dim=-1))
        elif self.mode == 'trans':
            out = self.fc_out(input_emb)
        else:
            out = input_emb
        return out
    ...
    def forward(self, input_emb, target_emb):
        ...
        out = self.get_out_emb(input_emb, target_emb)
        return out
...
class CapsuleNetwork(nn.Module):
    def __init__(self, dim_in, dim_out, bias=True, num_routing=3, num_caps=4, return_prob=False,
        ...
        self.layers = nn.ModuleList([OriginCaps(in_dim=dim_in, in_caps=1, num_caps=num_caps, dim_caps=dim_capsule,
                                                num_routing=num_routing)])
        for i in range(n_layers - 1):
            self.layers.append(OriginCaps(in_dim=dim_capsule, in_caps=num_caps, num_caps=num_caps, dim_caps=dim_capsule,
                                          num_routing=num_routing))
        self.bn_out = nn.BatchNorm1d(dim_capsule)
    ...
    def forward(self, x):
        ...
        # x = self.bn_in(x)
        x = x.unsqueeze(dim=1)
        # x, p = self.model(x)  # [N, Nc, dc] p is capsule probability
        # print(p)
        for i in range(self.n_layers - 1):
            x, p = self.layers[i](x)
        x, p = self.layers[self.n_layers-1](x)
        x_split = self.bn_out(x.transpose(1, 2)).transpose(1, 2)
        x = self.fc_out(x_split.reshape(-1, self.dim_out))
        # x = self.bn_out(x)
...
# class DGLCapsuleNetwork(nn.Module):
#     def __init__(self):
#         super(DGLCapsuleNetwork, self).__init__()
#
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, input_embs, masks):
        # h_0, c_0 = initial_state
        packed_input = pack_padded_sequence(input_embs, masks.sum(dim=-1).cpu().numpy(), batch_first=True)
...
if __name__ == '__main__':
    ...
    # print(pe(x).shape)
    pe = PositionalEncodingSC(128, batch_first=True)
