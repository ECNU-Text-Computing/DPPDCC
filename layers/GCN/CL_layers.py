...
class CommonCLLayer(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        # compute InfoNCE
        # loss12 = -torch.log(s12.diag().float()) + torch.log(s12.sum(1).float())
        # loss21 = -torch.log(s21.diag().float()) + torch.log(s21.sum(1).float())
...
def triplet_cl(zo, zp, zn, tau):
    # z [N, d]
    ho, hp, hn = zo.unsqueeze(dim=-2), zp.unsqueeze(dim=-2), zn.unsqueeze(dim=-2)
    pos = cos_sim(ho, hp, tau, norm=False).squeeze(dim=-1).squeeze(dim=-1)
    neg = cos_sim(ho, hn, tau, norm=False).squeeze(dim=-1).squeeze(dim=-1)
    # print(pos.shape, neg.shape)
    L_node = -torch.log(pos) + torch.log(pos + neg)
    return L_node.mean()
...
def node_drop(graph, prob, ntypes=None, selected_nodes=None):
    ...
    if count > 0:
        ...
        cur_p = torch.softmax(-graph.nodes['paper'].data['citations'][selected_index].float(),
                              dim=-1).numpy() * drop_rate
...
def augmenting_graphs_new(graphs, aug_configs, aug_methods=(('attr_drop',), ('edge_drop',)), time_length=10, copy=True):
    g1_list = []
    g2_list = []
    for graph in graphs:
        # g1, g2 = copy.deepcopy(graph), copy.deepcopy(graph)
        if copy:
            ...
            g1, g2 = graph, add_new_elements(graph)
        for t in range(time_length):
            # g1
            for aug_method in aug_methods[0]:
                aug_types = aug_configs[0][aug_method].get('types', None)
                selected_nodes = {}
                selected_ntypes = aug_types if aug_types is not None else graph.ntypes
                for ntype in selected_ntypes:
                    selected_nodes[ntype] = g1.nodes[ntype].data['snapshot_idx'] == t
                if torch.sum(selected_nodes['paper']).item() > 0:
                    cur_method = eval(aug_method)
                    g1 = cur_method(g1, aug_configs[0][aug_method]['prob'], aug_types,
                                    selected_nodes)
            # g2
            for aug_method in aug_methods[1]:
                aug_types = aug_configs[1][aug_method].get('types', None)
                selected_nodes = {}
                selected_ntypes = aug_types if aug_types is not None else graph.ntypes
                for ntype in selected_ntypes:
                ...
                if torch.sum(selected_nodes['paper']).item() > 0:
                    cur_method = eval(aug_method)
                    g2 = cur_method(g2, aug_configs[1][aug_method]['prob'], aug_types,
                                    selected_nodes)
        g1_list.append(g1)
        g2_list.append(g2)
